...
export const AddContactItem = (payload) => async (dispatch) => {
  try {
    const value = await axios.post('http://awtapi.softwarehouseparma.net/api/providers', payload, {
      headers: {
        'Content-Type': 'application/ld+json',
      }
    })
    dispatch(addContact(payload.id,payload.companyName,payload.address,payload.image,payload.taxCode,payload.vatNumber,payload.phone,payload.email,payload.contactName,payload.contactPhone,payload.contactEmail,payload.providerCode,payload.startDate,payload.paymentMethod,payload.creditLimit,payload.sdiCode,payload.invoiceCode,payload.website,payload.note,value['@id']));
  } catch (err) {
    throw new Error(err);
  }
};
...
export const fetchContacts = () => async (dispatch) => {
  try {
    const response = await axios.get('http://awtapi.softwarehouseparma.net/api/providers');
...
export const addContactsPage = (currentPage,keySearch) => async (dispatch) => {
  try {
    const response = await axios.get('http://awtapi.softwarehouseparma.net/api/providers?page=' + currentPage + '&companyName=' + keySearch);
