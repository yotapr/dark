...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      newObj[id] = {"provider" : provider, "orderDate": orderDate, "deliveryDate": deliveryDate, "realDeliveryDate": realDeliveryDate, "color": color, "note": note};
      dispatch(UpdateOrder({"provider" : provider, "orderDate": orderDate, "deliveryDate": deliveryDate, "realDeliveryDate": realDeliveryDate, "color": color, "note": note}))
      setJsonData(newObj);
    } else {
      const provider = event.target.provider.value;
      const orderDate = event.target.orderDate.value;
  ...
  const data2 = jsonData.map((prop, key) => {
    return {
      ...
      realDeliveryDate: prop.realDeliveryDate,
      color: prop.color,
      note: prop.note,
      actions: (
