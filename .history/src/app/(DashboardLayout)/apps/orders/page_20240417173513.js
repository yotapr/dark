...
const CustomReactTable = () => {
  ...
  const providers = useSelector((state) => state.providersReducer.providers);
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      console.log(newObj)
      dispatch(addOrder(name, materialCost, mechanicalDesignCost, electricalDesignCost, electricalWiringOnBoardTheMachine, panelWiringCost, mechanicalAssembly, mechanicalDesignCost, preparationCost, revenuePercentage, price, client, salesAgent))
      setJsonData(newObj);
    }
    setModal(!modal);
  };
  ...
  const data2 = jsonData.map((prop, key) => {
    return {
      id: key,
      provider: providers.find((singleProvider) => singleProvider['@id'] === prop[11]) && providers.find((singleProvider) => singleProvider['@id'] === prop[11]).companyName,
      actions: (
