...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    let clientText = ""
    let offerText = ""
    selectedClients.map((singleClient) => clientText = clientText + clients.find((singleContact) => singleContact['@id'] === singleClient.value) && clients.find((singleContact) => singleContact['@id'] === singleClient.value).companyName) + " "
    return {
      id: key,
      name: prop.name,
      clients: clientText,
