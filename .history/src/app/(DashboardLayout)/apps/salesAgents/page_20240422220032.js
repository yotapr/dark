...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      console.log(newObj)
      dispatch(addSalesAgent(name, offers, clients))
      //setJsonData(newObj);
    }
    setModal(!modal);
  };
  ...
  const data2 = salesAgents.map((prop, key) => {
    return {
      ...
      offers: offers.find((singleSalesAgent) => singleSalesAgent['@id'] === prop[12]) && salesAgents.find((singleSalesAgent) => singleSalesAgent['@id'] === prop[12]).name,
      actions: (
