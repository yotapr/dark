...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    ...
    return {
      id: key,
      name: prop.name,
      clients: prop.clients //clients.find((singleContact) => singleContact['@id'] === prop.clients) && clients.find((singleContact) => singleContact['@id'] === prop.clients).companyName
      ,
      offers: prop.offers //offers.find((singleOffers) => singleOffers['@id'] === prop.name) && offers.find((singleOffers) => singleOffers['@id'] === prop.offers).name
      ,
      actions: (
