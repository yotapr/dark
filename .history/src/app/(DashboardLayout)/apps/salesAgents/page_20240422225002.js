...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    ...
    //prop.clients.map((singleClient) => clientText += clients.find((singleContact) => singleContact['@id'] === singleClient.value) && clients.find((singleContact) => singleContact['@id'] === singleClient.value).companyName) + " "
    return {
      id: key,
      name: prop.name,
      clients: clientText,
