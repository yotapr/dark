...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    ...
    prop.offers.length > 0 && prop.offers.map((singleOffer) => { offersArray.push(singleOffer['@id'] !== undefined ? offersOptions.find((singleTotalOffer) => singleTotalOffer.value === singleOffer['@id']) : offersOptions.find((singleTotalOffer) => singleTotalOffer.value === singleOffer.value))})
    return {
      id: key,
      name: prop.name,
      clients: clientText,
