...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    return {
      id: key,
      name: prop.name,
      client: clients.find((singleContact) => singleContact['@id'] === prop.clients) && clients.find((singleContact) => singleContact['@id'] === prop.clients).companyName,
      offers: offers.find((singleOffers) => singleOffers['@id'] === prop.name) && singleOffers.find((singleOffers) => singleOffers['@id'] === prop.offers).name,
      actions: (
