...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      dispatch(UpdateSalesAgent(id, name, clients, offers ))
    } else {
      const name = event.target.name.value;
      const offers = selectedOffers.map((singleOffer) => singleOffer.value);
      const clients = selectedClients.map((singleClient) => singleClient.value);
      ...
      console.log(newObj)
      dispatch(addSalesAgent(salesAgents.length + 1, name, clients, offers))
      //setJsonData(newObj);
    }
    setModal(!modal);
  };
  ...
  const data2 = salesAgents.map((prop, key) => {
    ...
    let offersArray = []
    let clientsArray = []
    console.log(prop.clients.length)
    console.log(prop.offers.length)
    console.log(prop.offers)
    ...
    prop.clients.length > 0 && prop.clients.map((singleClient) => { clientsArray.push(clients.find((singleContact) => singleContact['@id'] === singleClient) && {value: clients.find((singleContact) => singleContact['@id'] === singleClient)['@id'], label: clients.find((singleContact) => singleContact['@id'] === singleClient).companyName })})
    prop.offers.length > 0 && prop.offers.map((singleOffer) => { offersArray.push(offers.find((singleTotalOffer) => singleTotalOffer['@id'] === singleOffer) && offers.find((singleTotalOffer) => singleTotalOffer['@id'] === singleOffer))})
    return {
      id: key,
      name: prop.name,
      clients: clientText,
          ...
          <Form onSubmit={(event) => handleSubmit(event)}>
            ...
            <FormGroup>
              ...
              <Select
                ...
                value={obj !== null ? obj.clientsArray : []}
              />
            </FormGroup>
            <FormGroup>
              ...
              <Select
                ...
                value={obj !== null ? obj.offersArray : []}
              />
            </FormGroup>
            <FormGroup>
