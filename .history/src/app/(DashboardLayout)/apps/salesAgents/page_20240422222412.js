...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    let clientText = ""
    let offerText = ""
    prop.clients.map((singleClient) => clientText += clients.find((singleContact) => singleContact['@id'] === singleClient) && clients.find((singleContact) => singleContact['@id'] === prop.singleClient).companyName) + " "
    return {
      id: key,
      name: prop.name,
      clients: clients.find((singleContact) => singleContact['@id'] === prop.clients) && clients.find((singleContact) => singleContact['@id'] === prop.clients).companyName,
      offers: offers.find((singleOffers) => singleOffers['@id'] === prop.name) && offers.find((singleOffers) => singleOffers['@id'] === prop.offers).name,
      actions: (
