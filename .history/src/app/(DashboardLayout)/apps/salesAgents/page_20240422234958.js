...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    ...
    prop.clients.length > 0 && prop.clients.map((singleClient) => { clientText += clients.find((singleContact) => singleContact['@id'] === singleClient.value) && clients.find((singleContact) => singleContact['@id'] === singleClient.value).companyName + " " })
    prop.offers.length > 0 && prop.offers.map((singleOffer) => { offerText += offers.find((singleTotalOffer) => singleTotalOffer['@id'] === singleOffer.value) && offers.find((singleTotalOffer) => singleTotalOffer['@id'] === singleOffer.value).name + " " })
