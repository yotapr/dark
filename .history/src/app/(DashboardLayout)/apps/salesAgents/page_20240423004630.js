...
const CustomReactTable = () => {
  ...
  const data2 = salesAgents.map((prop, key) => {
    ...
    prop.clients.length > 0 && prop.clients.map((singleClient) => { clientsArray.push(clients.find((singleContact) => singleContact['@id'] === singleClient) && singleClient)})
    prop.offers.length > 0 && prop.offers.map((singleOffer) => { offersArray.push(offersOptions.find((singleTotalOffer) => singleTotalOffer.value === singleOffer) && singleTotalOffer)})
    return {
      id: key,
      name: prop.name,
      clients: clientText,
