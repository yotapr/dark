...
import { fetchAppointments, addNewAppointment, fetchUsers, editAppointment } from "@/store/apps/appointments/AppointmentSlice";
...
const CalendarApp = () => {
  ...
  const editEvent = (event) => {
    ...
    setUser(newEditEvent.user);
    setId(newEditEvent.id);
    setAppointmentStart(new Date(newEditEvent.start).getFullYear() + "-" + (new Date(newEditEvent.start).getMonth() < 9 ? "0" : "") + (new Date(newEditEvent.start).getMonth() + 1) + "-" + (new Date(newEditEvent.start).getDate() < 10 ? "0" : "") + new Date(newEditEvent.start).getDate() + "T" + (new Date(newEditEvent.start).getHours() < 10 ? "0" : "") + new Date(newEditEvent.start).getHours() + ":" + (new Date(newEditEvent.start).getMinutes() < 10 ? "0" : "") + new Date(newEditEvent.start).getMinutes());
    setAppointmentEnd(new Date(newEditEvent.end).getFullYear() + "-" + (new Date(newEditEvent.end).getMonth() < 9 ? "0" : "") + (new Date(newEditEvent.end).getMonth() + 1) + "-" + (new Date(newEditEvent.end).getDate() < 10 ? "0" : "") + new Date(newEditEvent.end).getDate() + "T" + (new Date(newEditEvent.end).getHours() < 10 ? "0" : "") + new Date(newEditEvent.end).getHours() + ":" + (new Date(newEditEvent.end).getMinutes() < 10 ? "0" : "") + new Date(newEditEvent.end).getMinutes());
    setUpdate(event);
  };
    ...
    setCalEvents(
      calevents.map((elem) => {
        if (elem.id === update.id) {
          return { ...elem, title, color, start: appointmentStart, end: appointmentEnd, id, user };
        }
        return elem;
      })
    );
    let editedAppointment = await calevents.find((elem) => elem.id === update.id);
    editedAppointment = {...editedAppointment, appointmentStart: editedAppointment.start, appointmentEnd: editedAppointment.end}
