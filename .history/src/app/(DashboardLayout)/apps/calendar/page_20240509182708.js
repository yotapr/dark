    ...
    setCalEvents(
      calevents.map((elem) => {
        if (elem.id === update.id) {
          return { ...elem, title, color, start: appointmentStart, end: appointmentEnd, appointmentStart, appointmentEnd, id, user };
        }
        return elem;
      })
    );
    dispatch(editAppointment(calevents.find((elem) => elem.id === update.id)))
    setOpen(false);
    setAppointmentStart("");
    setAppointmentEnd("");
    setTitle("");
