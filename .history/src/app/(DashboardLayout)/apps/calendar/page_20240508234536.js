...
const CalendarApp = () => {
  ...
  const addNewEventAlert = (slotInfo) => {
    setOpen(true);
    setSlot(slotInfo);
    setAppointmentStart(new Date(slotInfo.start).getFullYear() + "-" + (new Date(slotInfo.start).getMonth() < 9 ? "0" : "") + (new Date(slotInfo.start).getMonth() + 1) + "-" + (new Date(slotInfo.start).getDate() < 9 ? "0" : "") + new Date(slotInfo.start).getDate() + "T" + (new Date(slotInfo.start).getHours() < 10 ? "0" : "") + new Date(slotInfo.start).getHours() + ":" + (new Date(slotInfo.start).getMinutes() < 10 ? "0" : "") + new Date(slotInfo.start).getMinutes());
    setAppointmentEnd(slotInfo.end);
  };
