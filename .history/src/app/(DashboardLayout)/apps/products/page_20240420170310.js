...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      console.log(newObj[event.target.id.value])
      newObj[event.target.id.value] = {id,productId,name,category,description,stockQuantity,lowLevel,availableQuantity,allocatedQuantity,restockDate,lastRestockQuantity,purchasePrice,sellPrice,weight,dimensions,providers,note};
      dispatch(UpdateProduct(newObj[event.target.id.value]))
      setJsonData(newObj);
    } else {
      const productId = event.target.productId.value;
      const name = event.target.name.value;
