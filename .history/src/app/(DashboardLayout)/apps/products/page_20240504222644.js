...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      dispatch(UpdateProduct({id,productId, name, category, description, stockQuantity, lowLevel, availableQuantity, allocatedQuantity, restockDate, lastRestockQuantity, purchasePrice, sellPrice, weight, dimensions, provider, note}))
      setJsonData(newObj);
    } else {
      const productId = event.target.productId.value;
      const name = event.target.name.value;
