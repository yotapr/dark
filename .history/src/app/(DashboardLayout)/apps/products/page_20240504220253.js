...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      const note = event.target.note.value;
      const newObj = JSON.parse(JSON.stringify(jsonData));
      newObj.push({"productId" : productId, "name": name, "category": category, "description": description, "stockQuantity": stockQuantity, "lowLevel": lowLevel, "availableQuantity": availableQuantity, "allocatedQuantity": allocatedQuantity, "restockDate": restockDate, "lastRestockQuantity": lastRestockQuantity, "purchasePrice": purchasePrice, "sellPrice": sellPrice, "weight": weight, "dimensions": dimensions, "provider": providers, "note": note});
      console.log(newObj)
      dispatch(AddProductsItem({productId, name, category, description, stockQuantity, lowLevel, availableQuantity, allocatedQuantity, restockDate, lastRestockQuantity, purchasePrice, sellPrice, weight, dimensions, providers, note}))
      setJsonData(newObj);
    }
    setModal(!modal);
  };
