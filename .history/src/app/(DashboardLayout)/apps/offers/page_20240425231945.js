...
const CustomReactTable = () => {
  ...
  const data2 = jsonData.map((prop, key) => {
    return {
      id: key,
      name: prop[0],
      client: clients.length > 0 && clients.map((singleClient) => { clientText += clients.find((singleContact) => singleContact['@id'] === singleClient) && clients.find((singleContact) => singleContact['@id'] === singleClient).companyName + " " }),
