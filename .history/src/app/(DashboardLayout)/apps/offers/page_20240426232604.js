...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      dispatch(UpdateOffer({name, materialCost, mechanicalDesignCost, electricalDesignCost, electricalWiringOnBoardTheMachine, panelWiringCost, mechanicalAssembly, mechanicalDesignCost, preparationCost, revenuePercentage, price, client, salesAgent, remoteId}))
    } else {
      const id = event.target.id.value;
      const name = event.target.name.value;
      const materialCost = event.target.materialCost.value;
      ...
      <Modal isOpen={modal} toggle={toggle.bind(null)}>
        ...
        <ModalBody>
          <Form onSubmit={(event) => handleSubmit(event)}>
            ...
            {obj !== null && <Input type="hidden" name="remoteId" id="remoteId" defaultValue={obj.remoteId} />}            {obj !== null && <Input type="hidden" name="id" id="id" defaultValue={obj.id} />}
            <FormGroup>
