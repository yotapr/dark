...
const CustomReactTable = () => {
  ...
  const data2 = jsonData.map((prop, key) => {
    let clientText = ""
    clientText = prop.clients.length > 0 && prop.clients.map((singleClient) => { clientText += clients.find((singleContact) => singleContact['@id'] === singleClient) && clients.find((singleContact) => singleContact['@id'] === singleClient).companyName + " " })
    return {
      id: key,
      name: prop[0],
      client: clients.find((singleContact) => singleContact['@id'] === prop[11]) && clients.find((singleContact) => singleContact['@id'] === prop[11]).companyName,
