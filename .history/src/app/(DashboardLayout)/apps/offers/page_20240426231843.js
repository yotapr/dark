...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      dispatch(UpdateOffer({name, materialCost, mechanicalDesignCost, electricalDesignCost, electricalWiringOnBoardTheMachine, panelWiringCost, mechanicalAssembly, mechanicalDesignCost, preparationCost, revenuePercentage, price, client, salesAgent, id}))
    } else {
      const name = event.target.name.value;
      const materialCost = event.target.materialCost.value;
      const mechanicalDesignCost = event.target.mechanicalDesignCost.value;
