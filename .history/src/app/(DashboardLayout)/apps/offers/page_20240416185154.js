...
const CustomReactTable = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (event.target.id.value !== undefined) {
      ...
      const newObj = JSON.parse(JSON.stringify(jsonData));
      newObj.push([name, materialCost, mechanicalDesignCost, electricalDesignCost, electricalWiringOnBoardTheMachine, panelWiringCost, mechanicalAssembly, mechanicalDesignCost, preparationCost, revenuePercentage]);
      console.log(newObj)
      dispatch(addOffer([name, designation, location, age]))
      setJsonData(newObj);
    }
    setModal(!modal);
  };
