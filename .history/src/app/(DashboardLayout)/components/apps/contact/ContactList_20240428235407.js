  ...
  return (
    <Nav>
      <Modal isOpen={modalDelete} toggle={() => setModalDelete(!modalDelete)}>
        ...
        <ModalBody>
          <Button style={{margin: "5px"}} color="danger" onClick={() => {dispatch(deleteContactItem((contact['@id'] ? contact['@id'] : contact.remoteId),contact.id)); setModalDelete(!modalDelete)}}>       Rimuovi
          </Button>
          <Button
            color="secondary"
            className="ml-1"
      ...
      {contacts.map((contact,index) => (
        <ContactListItem
          ...
          onDeleteClick={() => setModalDelete(!modalDelete)}
