...
const ContactDetails = () => {
  ...
  const [startDate, setStartDate] = React.useState(useSelector((state) => state.contactsReducer.contacts[state.contactsReducer.contactContent].startDate))

  React.useEffect(() => {
    //setStartDate(new Date(startDate).getFullYear() + "/" + (new Date(startDate).getMonth() + 1) + "/" + new Date(startDate).getFullYear())
  }),[]
