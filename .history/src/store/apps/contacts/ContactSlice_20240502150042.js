...
export const addContactsPage = (currentPage) => async (dispatch) => {
  try {
    const response = await axios.get('http://awtapi.softwarehouseparma.net/api/clientss?page=' + currentPage);
...
export const searchContacts = (keySearch,currentPage) => async (dispatch) => {
  try {
    const response = await axios.get('http://awtapi.softwarehouseparma.net/api/clientss?page=' + currentPage + 'companyName=' + keySearch);
    dispatch(getContacts(response.data['hydra:member']));
    response.data['hydra:view'] && dispatch(setPageData(response.data['hydra:view']))
  } catch (err) {
    throw new Error(err);
