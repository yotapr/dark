...
export const SalesAgentsSlice = createSlice({
  ...
  reducers: {
    ...
    DeleteSalesAgent: (state, action) => {
      ...
      state.salesAgents.map((singleSalesAgent) => console.log(singleSalesAgent.remoteId))
      const index = state.salesAgents.findIndex((singleSalesAgent) => singleSalesAgent.remoteId === action.payload);
      console.log(index)
      index !== 1 && state.salesAgents.splice(index, 1);
    },
...
export const deleteSalesAgentsItem = (remoteId) => async (dispatch) => {
  try {
    const response = await axios.delete('http://awtapi.softwarehouseparma.net' + remoteId);
    dispatch(DeleteSalesAgent(remoteId));
  } catch (err) {
    throw new Error(err);
  }
};
