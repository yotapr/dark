  ...
  reducers: {
    ...
    UpdateSalesAgent: {
      reducer: async (state, action) => {
        const value = await axios.patch('http://awtapi.softwarehouseparma.net' + action.payload.remoteId, action.payload, {
          headers: {
              'Content-Type': 'application/merge-patch+json',
          }
        })
        console.log(value.data)
        state.salesAgents[parseInt(action.payload.id)] = value.data
        //state.salesAgents = action.payload
      },
      prepare: (
        id,
