  ...
  reducers: {
    ...
    UpdateSalesAgent: {
      reducer: async (state, action) => {
        await axios.patch('http://awtapi.softwarehouseparma.net' + action.payload.remoteId, action.payload, {
          headers: {
              'Content-Type': 'application/merge-patch+json',
          }
        })//.then(() => dispatch(getSalesAgents(response.data['hydra:member'])))
        //console.log(state.salesAgents[parseInt(action.payload.id)])
        state.salesAgent[parseInt(action.payload.id)] = action.payload
      },
      prepare: (
        id,
        name,
