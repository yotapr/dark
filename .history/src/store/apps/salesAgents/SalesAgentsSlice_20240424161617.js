...
export const SalesAgentsSlice = createSlice({
  ...
  reducers: {
    ...
    DeleteSalesAgent: (state, action) => {
      const index = state.salesAgents.findIndex((salesAgent) => salesAgent.remoteId === action.payload);
...
export const AddSalesAgentsItem = (payload) => async (dispatch) => {
  try {
    ...
    //const response = await axios.get('http://awtapi.softwarehouseparma.net/api/sales_agentss');
    dispatch(addSalesAgent(payload.id,payload.name,payload.clients,payload.offers,value['@id']));
  } catch (err) {
    throw new Error(err);
  }
};
