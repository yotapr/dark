...
export const SalesAgentsSlice = createSlice({
  ...
  reducers: {
    ...
    setPageData: {
      reducer: (state, action) => {
        console.log(action.payload.pageData["hydra:last"].match(/(\d+)/)[0])

        state.salesAgents.totalPage = action.payload.pageData["hydra:last"].match(/(\d+)/)[0]
        //state.salesAgents.currentPage = action.payload.pageData["@id"].match(/(\d+)/)
        //state.salesAgents.push(action.payload);
      },
      prepare: (
        pageData
