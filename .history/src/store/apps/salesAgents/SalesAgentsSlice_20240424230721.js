  ...
  reducers: {
    ...
    setPageData: {
      reducer: (state, action) => {
        ...
        state.currentPage = action.payload.pageData["@id"].match(/(\d+)/)[0]
        if (action.payload.pageData["hydra:next"]) state.nextPage = action.payload.pageData["hydra:next"].match(/(\d+)/)[0]
        if (action.payload.pageData["hydra:previous"]) state.prevPage = action.payload.pageData["hydra:previous"].match(/(\d+)/)[0]
        //state.salesAgents.push(action.payload);
      },
      prepare: (
        pageData
